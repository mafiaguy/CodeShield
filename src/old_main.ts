...
interface LanguageOption {
  name: string;
  dockerImage: string;
  commandTemplate: (codePath: string, filesToScan: string, scanWholeCode: boolean, additionalOptions: string) => string;
}
...
class SASTWrapper {
  async selectLanguage(): Promise<LanguageOption> {
    const { languageName } = await inquirer.prompt<{ languageName: string }>([
      {
        type: 'list',
        name: 'languageName',
  ...
  async getCodePath(): Promise<string> {
    const { codePath } = await inquirer.prompt<{ codePath: string }>([
      {
        type: 'input',
        name: 'codePath',
        message: 'Please enter the absolute path of the code to be scanned:',
        validate: (input: string) => input ? true : 'Code path cannot be empty',
      }
    ]);
    return codePath;
  }

  async shouldScanWholeCode(): Promise<boolean> {
    const { scanWholeCode } = await inquirer.prompt<{ scanWholeCode: boolean }>([
      {
        type: 'confirm',
        name: 'scanWholeCode',
  ...
  async getSpecificFiles(): Promise<string> {
    const { specificFiles } = await inquirer.prompt<{ specificFiles: string }>([
      {
        type: 'input',
        name: 'specificFiles',
        message: 'Please enter the specific file paths (comma separated):',
        validate: (input: string) => input ? true : 'You must enter at least one file',
      }
    ]);
    return specificFiles;
  }

  async getAdditionalBrakemanOptions(): Promise<string> {
    const { additionalOptions } = await inquirer.prompt<{ additionalOptions: string }>([
      {
        type: 'input',
        name: 'additionalOptions',
        message: 'Enter any additional Brakeman options you want to use (or leave blank for none):',
      }
    ]);
    return additionalOptions.trim();
  }

  async pullDockerImage(language: LanguageOption) {
    console.log(`Pulling Docker image for ${language.name}...`);
    await execPromise(`docker pull ${language.dockerImage}`);
  }

  async runDockerCommand(language: LanguageOption, codePath: string, filesToScan: string, scanWholeCode: boolean, additionalOptions: string): Promise<string> {
  ...
  async run() {
    try {
      ...
      const additionalOptions = await this.getAdditionalBrakemanOptions();
